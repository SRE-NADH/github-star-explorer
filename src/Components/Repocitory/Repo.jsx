...
//  const [allChangesPerContributorData,setAllChangesPerContributerData] = useState([]);
  ...
  async function fetchAdditionOrDeletionData(){
   try{
    let response = await axios.get(`https://api.github.com/repos/${data.owner.login}/${data.name}/stats/code_frequency`);
  ...
  return (
    <>
    <div style={{...styles.repo,flexDirection:isSmallScreen?'column':'row'}}>
      <div style={!isSmallScreen?{display:'flex',alignItems:'center'}:{display:'flex',justifyContent:'center'}}>
      <Avatar
      ...
      <div  onClick={handleGraphs} style={{...styles.repoDetails,textAlign: isSmallScreen?'center':""}}>
        <Typography variant='h5'>{data.name}</Typography>
        <Typography>
         {data.description}
        </Typography>
        <div style={{...styles.extra,flexDirection:isSmallScreen?'column':'row'}}>
      ...
      <div style={{...styles.dropArrow,justifyContent:isSmallScreen?'center':''}}>
       {show?<IoIosArrowDown/>:<IoIosArrowForward/>} 
      </div>
    </div>
    {
      show && <div ><Chart data={additionOrDeletionData}/></div>      
    }
    </>
  );
};
